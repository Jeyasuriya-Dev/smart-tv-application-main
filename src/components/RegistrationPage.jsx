...
export default function RegistrationPage() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (regData?.status === "success") {
				...
				if (existData?.device_status && existData?.client_status && !existData.isexpired) {
					toast.success("Device registered successfully ");
					setDeviceDetails(existData); // save device details to store
	...
	return (
		<div>
			<div className="container">
				<div className='contents-container'>
					<img src={applogo} alt="IQ World Logo" className="logo" />
					<p className="tagline">INDIA KA IQ</p>
					<div className='head'>
						<h2>New Device Registration</h2>
					</div>
					...
					<div className="form-group">
						<label htmlFor="unique-number">Unique Number</label>
						<input type="text" id="unique-number" onChange={handleChange} style={{ textTransform: "uppercase" }} />
					</div>
					<div className='button-container'>
